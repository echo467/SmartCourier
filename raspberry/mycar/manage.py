...
# important
    ...
    if model_path:
        ...
        V.add(TriggeredCallback(model_path, model_reload_cb), inputs=["modelfile/reload"], run_condition="ai_running_no")
        #model_path_2 = './'
        #V.add(TriggeredCallback(model_path_2, model_reload_cb), inputs=["modelfile/reload"], run_condition="ai_running")
    ...
    class DriveMode:
        def run(self, mode, 
                    user_angle, user_throttle, user_arrival,
                    pilot_angle, pilot_throttle, pilot_arrival):
            if mode == 'user': 
                return user_angle, user_throttle, user_arrival
            
            elif mode == 'local_angle':
                return pilot_angle, user_throttle, user_arrival
            
            else: 
                return pilot_angle, pilot_throttle * cfg.AI_THROTTLE_MULT, pilot_arrival
        
    V.add(DriveMode(), 
          inputs=['user/mode', 'user/angle', 'user/throttle','user/arrival',
                  'pilot/angle', 'pilot/throttle', 'pilot/arrival'],
          outputs=['angle', 'throttle', 'arrival'])

    # to decide whether to stop when ai_running
    class Locator:
        def run(self, arrive_signal):
            # stop_signal default False
            if arrive_signal>0:
                print('arrival is {}'.format(arrive_signal))
                return True
            else:
                return False
            #test
    ...
    V.add(Locator(), inputs=['arrival'], outputs=['stop_signal'],run_condition="ai_running")
...
#zd
        ...
        V.add(steering, inputs=['angle', 'stop_signal'])
        V.add(throttle, inputs=['throttle', 'stop_signal'])
